...
- [x] **@ 原语**：输入框支持 @记忆 / @技能 / @文件 / @Git工作数据，统一所有能力的上下文入口（引用随消息持久化，多轮对话持续在场）
- [x] **Git 上下文接入对话**："帮我写周报 @本周提交"一句话完成，@Git 即第一形态
- [ ] **定时自动化**：日报/周报定时生成 + 本地通知/推送（暂缓）
- [ ] **记忆对象化**：记忆可被 @ 引用、可置顶、带预算（@ 引用已支持，置顶/预算待做）
